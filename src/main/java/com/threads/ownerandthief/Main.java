...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        initializeOwnersAndThieves(home, originListOfThings,list);
    ...
    public static void initializeOwnersAndThieves(Home home, List<Thing> originListOfThings, List <Future<List>> list){
        ExecutorService executor = Executors.newFixedThreadPool(numberOfOwners + numberOfThieves);
        Random random = new Random();

        for (int i = 0; i < numberOfOwners; i++){
            Owner owner = new Owner(home, barrier);
        ...
        for (int i = 0; i < numberOfThieves; i++){
            Callable<List> callableThieves = new Thief(home, barrier);
